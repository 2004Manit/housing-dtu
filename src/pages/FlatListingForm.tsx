...
// Flat Amenities
const flatAmenities = [
  { id: 'ac-rooms', label: 'AC Rooms', icon: Wind, color: { bg: 'bg-cyan-500/10', text: 'text-cyan-400' } },
  { id: 'separate-almirah-flat', label: 'Separate Almirah', icon: Archive, color: { bg: 'bg-purple-500/10', text: 'text-purple-400' } },
  { id: 'mattress-pillows', label: 'Mattress & Pillows', icon: Home, color: { bg: 'bg-pink-500/10', text: 'text-pink-400' } },
  { id: 'water-purifier', label: 'Water Purifier', icon: Droplets, color: { bg: 'bg-blue-500/10', text: 'text-blue-400' } },
  { id: 'modular-kitchen', label: 'Modular Kitchen', icon: UtensilsCrossed, color: { bg: 'bg-amber-500/10', text: 'text-amber-400' } },
  { id: 'wifi', label: 'WiFi', icon: Wifi, color: { bg: 'bg-indigo-500/10', text: 'text-indigo-400' } },
  { id: 'study-table', label: 'Study Table', icon: FileText, color: { bg: 'bg-green-500/10', text: 'text-green-400' } },
  { id: 'balcony', label: 'Balcony', icon: Home, color: { bg: 'bg-emerald-500/10', text: 'text-emerald-400' } },
  { id: 'common-sitting-area', label: 'Common Sitting Area', icon: Users, color: { bg: 'bg-teal-500/10', text: 'text-teal-400' } },
  { id: 'induction-stove', label: 'Induction Stove', icon: Flame, color: { bg: 'bg-orange-500/10', text: 'text-orange-400' } },
  { id: 'blower', label: 'Blower', icon: Wind, color: { bg: 'bg-sky-500/10', text: 'text-sky-400' } },
  { id: 'fridge', label: 'Fridge', icon: Refrigerator, color: { bg: 'bg-violet-500/10', text: 'text-violet-400' } },
  { id: 'microwave', label: 'Microwave', icon: Microwave, color: { bg: 'bg-rose-500/10', text: 'text-rose-400' } },
  { id: 'washroom', label: 'Washroom', icon: DoorOpen, color: { bg: 'bg-lime-500/10', text: 'text-lime-400' } },
  { id: 'geyser', label: 'Geyser', icon: Flame, color: { bg: 'bg-red-500/10', text: 'text-red-400' } },
];

const FlatListingForm = () => {
  ...
  const steps = [
    ...
    { id: 2, title: "Details", icon: MessageCircle },
    { id: 3, title: "Amenities", icon: Wind },
    { id: 4, title: "Media & Location", icon: MapPin },
  ];
    ...
    switch (step) {
      case 0:
        ...
        if (!formData.name.trim()) {
          newErrors.name = 'Please add name of your flat/property';
          isValid = false;
        }
        break;
      case 1:
        newErrors.rent = '';
        if (!formData.rent) {
          newErrors.rent = 'Price can\'t be empty';
          isValid = false;
        }
        break;
      case 2:
        newErrors.whatsappNumber = '';
        newErrors.currentFlatmates = '';
        newErrors.flatmatesRequired = '';
        newErrors.brokerage = '';
        newErrors.bhk = '';
        ...
        if (!formData.currentFlatmates) {
          newErrors.currentFlatmates = 'Please enter number of current flatmates';
          isValid = false;
        }
        if (!formData.flatmatesRequired) {
          newErrors.flatmatesRequired = 'Please enter flatmates required';
          isValid = false;
        }
        if (!formData.brokerage) {
          newErrors.brokerage = 'Please select brokerage option';
          isValid = false;
        }
        if (!formData.bhk) {
          newErrors.bhk = 'Please enter BHK';
          isValid = false;
        }
        break;
      case 3:
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      console.log('Starting flat submission for user:', user.id);

      // Step 2: Upload images
      console.log('Uploading images...');
      const imageUrls = await uploadImages(formData.images, user.id, 'Flat');
      ...
      if (formData.video) {
        ...
        videoUrl = await uploadVideo(formData.video, user.id, 'Flat');
      ...
      const submissionData = {
        ...
        current_flatmates: parseInt(formData.currentFlatmates),
        flatmates_required: formData.flatmatesRequired,
        brokerage_required: formData.brokerage === 'brokerage-applicable',
        bhk_config: formData.bhk + 'BHK',

        // Amenities (Flat doesn't have facilities, only amenities)
        selected_facilities: [], // Empty for flats
      ...
      toast({
        ...
        description: "Your flat listing has been submitted for review.",
      });

      // Step 7: Close dialog and navigate
      setShowConfirmDialog(false);
      navigate('/flat-listing-success', {
        state: {
          formData: formData,
  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-6 py-16 sm:py-20 max-w-4xl relative z-10">
        ...
        {/* Form Steps - RESPONSIVE CARD PADDING */}
        <AnimatePresence mode="wait">
          ...
          >
            {/* Step 0: Basic Info - RESPONSIVE */}
            {currentStep === 0 && (
              <div className="space-y-4 sm:space-y-6">
                <div className="text-center mb-6 sm:mb-8">
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-2 px-2">What's the name of your flat/property?</h2>
                ...
                <div>
                  ...
                  <Input
                    type="text"
                    placeholder="e.g., Cozy 2BHK Apartment, Modern Flat..."
                    value={formData.name}
            ...
            {/* Step 1: Pricing - RESPONSIVE */}
            {currentStep === 1 && (
              <div className="space-y-4 sm:space-y-6">
                <div className="text-center mb-6 sm:mb-8">
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-2 px-2">What's the monthly rent?</h2>
                  <p className="text-white/60 text-sm sm:text-base px-4">Tell us the monthly rent for your flat</p>
                </div>
                <div>
                  <label className="text-white/80 text-xs sm:text-sm font-medium mb-2 block flex items-center gap-2">
                  ...
                  {errors.rent && (
                    <p className="text-red-500 text-xs sm:text-sm mt-2">{errors.rent}</p>
                  )}
                </div>
              </div>
            )}
            ...
            {currentStep === 2 && (
              <div className="space-y-4 sm:space-y-6">
                <div className="text-center mb-6 sm:mb-8">
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-2 px-2">Tell us more details</h2>
                  <p className="text-white/60 text-sm sm:text-base px-4">Help tenants know about your flat</p>
                </div>
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <label className="text-white/80 text-xs sm:text-sm font-medium mb-2 block flex items-center gap-2">
                    ...
                    <Input
                      ...
                      className="bg-white/5 border-white/20 text-white placeholder:text-white/40 text-base sm:text-lg py-3 sm:py-4 md:py-6 focus:border-blue-400 focus:ring-blue-400"
                      maxLength={10}
                    />
                  ...
                  <div>
                    ...
                    <Textarea
                      placeholder="Tell us more about your flat..."
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className="bg-white/5 border-white/20 text-white placeholder:text-white/40 text-base sm:text-lg py-3 sm:py-4 md:py-6 focus:border-green-400 focus:ring-green-400"
                      min="0"
                    />
                    {errors.currentFlatmates && (
                      <p className="text-red-500 text-xs sm:text-sm mt-2">{errors.currentFlatmates}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-white/80 text-xs sm:text-sm font-medium mb-2 block flex items-center gap-2">
                    ...
                    <Input
                      ...
                      value={formData.flatmatesRequired}
                      onChange={(e) => setFormData({ ...formData, flatmatesRequired: e.target.value })}
                      className="bg-white/5 border-white/20 text-white placeholder:text-white/40 text-base sm:text-lg py-3 sm:py-4 md:py-6 focus:border-green-400 focus:ring-green-400"
                    />
                    {errors.flatmatesRequired && (
                      <p className="text-red-500 text-xs sm:text-sm mt-2">{errors.flatmatesRequired}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-white/80 text-xs sm:text-sm font-medium mb-2 block flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                      {['zero-brokerage', 'brokerage-applicable'].map((option) => (
                        <motion.button
                          key={option}
                    ...
                    {errors.brokerage && (
                      <p className="text-red-500 text-xs sm:text-sm mt-2">{errors.brokerage}</p>
                    )}
                  </div>
                  <div>
                    <label className="text-white/80 text-xs sm:text-sm font-medium mb-2 block flex items-center gap-2">
                    ...
                    />
                    {errors.bhk && (
                      <p className="text-red-500 text-xs sm:text-sm mt-2">{errors.bhk}</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            {/* Step 3: Flat Amenities - RESPONSIVE */}
            {currentStep === 3 && (
              <div className="space-y-4 sm:space-y-6">
                <div className="text-center mb-6 sm:mb-8">
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-2 px-2">Flat Amenities</h2>
                  <p className="text-white/60 text-sm sm:text-base px-4">What amenities does your flat have?</p>
                </div>
            ...
            {/* Step 4: Media & Location - RESPONSIVE */}
            {currentStep === 4 && (
              <div className="space-y-4 sm:space-y-6">
                <div className="text-center mb-6 sm:mb-8">
                    ...
                    <div className="border-2 border-dashed border-white/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 text-center hover:border-green-400 transition-colors cursor-pointer bg-white/5">
                      ...
                      {formData.images.length === 0 ? (
                        <label htmlFor="image-upload" className="cursor-pointer">
                          ...
                          <p className="text-white/40 text-xs sm:text-sm">Upload multiple photos of your flat (Max 6)</p>
                        </label>
                      ) : (
                    ...
                    <div className="border-2 border-dashed border-white/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 text-center hover:border-green-400 transition-colors cursor-pointer bg-white/5">
                      ...
                      {!formData.video ? (
                        <label htmlFor="video-upload" className="cursor-pointer">
                          ...
                          <p className="text-white/40 text-xs sm:text-sm">Upload a video tour of your flat (Max 50 MB)</p>
                        </label>
                      ) : (
                        <div className="space-y-3 sm:space-y-4">
                          {/* Video Preview */}
        ...
        {/* Fun Encouragement Message - RESPONSIVE */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <p className="text-white/50 text-xs sm:text-sm px-4">
            ...
            {currentStep === 2 && "📝 Great! Tell us more details"}
            {currentStep === 3 && "✨ Awesome! Show off your amenities"}
            {currentStep === 4 && "🎊 Final step! You're almost done"}
          </p>
        </motion.div>
